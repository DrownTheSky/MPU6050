...
static void delay1_ms(uint16_t time);
/*****************************************************************************
...
{
	I2C_WriteByte(MPU6050Addr,reg,data);
		return 1;
}
...
{
	uint8_t Temp = 0;
	Temp = I2C_ReadByte(MPU6050Addr,reg);
	*buf = Temp;
		return 1;
}
...
{
	I2C_WriteBuffer(MPU6050Addr,reg,buf,len);
		return 1;
}
...
{
	I2C_ReadBuffer(MPU6050Addr,reg,buf,len);
		return 1;
}
...
{ 
	...
	{
		//print_uart2("\rMPU6050 no connect...\r\n");
	}
}
